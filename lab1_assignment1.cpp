...
vector<Process> nonPreemptiveSJF(vector<Process> processes)
{
    ...
    while (completed < n)
    {
        ...
        int shortestBurst = numeric_limits<int>::max(); // Initialized to max value
...
//  but now we implemented considering shortest burst time is scheduled to execute first which is
//  shortest Remaining burst time first method after checking priority. In a word , you should consider two things in
//  this algorithm priority at first and then based on priority shortest burst time is scheduled to execute first.

// NEW ALGO ( combination of Non-Preemptive Priority Scheduling + non preemptive SJF Scheduling )
        ...
        for (int i = 0; i < n; i++)
        {
            if (processes[i].arrivalTime <= currentTime && processes[i].remainingTime > 0)
            {
                ...
                if (bestProcessIdx == -1 || processes[i].priority < processes[bestProcessIdx].priority ||
                    (processes[i].priority == processes[bestProcessIdx].priority && processes[i].remainingTime < min_RemainingTime))
                {
                    min_RemainingTime = processes[i].remainingTime;
                    bestProcessIdx = i;
                }
            }
        }
...
void compareAllAlgorithms()
{
    ...
    cout << "+-------------------------+------------------+------------------+---------------------+\n";
    cout << "| FCFS                    | " << setw(16) << fixed << setprecision(5) << fcfsResponseTime << " | " << setw(16) << fcfsWaitingTime << " | " << setw(19) << fcfsTurnaroundTime << " |\n";
